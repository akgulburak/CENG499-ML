...
def calculate_split_values(data, labels, num_classes, attr_index, heuristic_name):
    ...
    for i in range(data.shape[0]-1):
        ...
        if heuristic_name=="info_gain":
            heur_val = info_gain(parent,helper_class(labels[0:i+1],num_classes),helper_class(labels[i+1:],num_classes))

        elif heuristic_name=="avg_gini_index":

            heur_val = avg_gini_index(helper_class(left_labels[0:i+1],num_classes),helper_class(right_labels[i+1:],num_classes))
